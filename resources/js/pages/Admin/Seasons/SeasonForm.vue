...
const fetchViewData = () => {
    axios.post(route('admin.seasons.preview'), { title: form.title, content: form.content, change_notes: form.change_notes }).then((response) => {
...
<template>
    ...
    <Card>
        <CardHeader>
            ...
            <CardDescription>
                Create and Edit Season Information
                <span class="text-destructive" v-if="!props.season"><br />Make sure you want an entirely NEW Season. <br />
                    If you just want to update or change an existing Season, you need to edit it.</span>
            </CardDescription>
        </CardHeader>
        <CardContent>
            <form @submit.prevent>
                <div class="grid items-center w-full gap-4">
                    <div class="flex flex-col space-y-1.5">
                        ...
                        <Input id="title" type="text" required autofocus :tabindex="1" autocomplete="title" v-model="form.title" placeholder="Season Title" />
                    ...
                    <div class="flex flex-col space-y-1.5" v-if="(props.season && props.season?.published_at) || props.season?.approval?.change_notes">
                        <DraggableContent
                            v-if="viewData"
                        ...
                        <DrawerHeader>
                            ...
                            <DrawerDescription>
                                <div class="mx-auto max-w-lg mt-2 container overflow-y-auto">
                                    ...
                                    <div class="flex items-center mb-4 space-x-2" v-if="hasPermission('approve_season')">
                                        <Switch id="approve-directly" v-model="form.approve_directly" />
                                        <Label for="approve-directly">Approve Directly</Label>
                                    </div>
                                    <div class="flex items-center mb-4 space-x-2" v-if="hasPermission('publish_season')">
