...
<template>
    ...
    <Card>
        <CardHeader>
            <CardTitle>{{ props.page ? 'Edit' : 'New' }} Page</CardTitle>
            <CardDescription>
                <span class="text-destructive" v-if="!props.page">
                    You are creating a new Page item. To update an existing one, find it in the list and click Edit.
                </span>
                <span v-else>Editing: {{ props.page.title }}</span>
            </CardDescription>
        </CardHeader>
        <CardContent>
            <form @submit.prevent class="space-y-6">
                <div class="space-y-4">
                    <div class="space-y-2">
                        ...
                        <Input id="title" type="text" required autofocus autocomplete="title" v-model="form.title" placeholder="Page Title" />
                ...
                <Collapsible>
                    ...
                    <CollapsibleContent class="space-y-4 pt-4">
                        <div class="space-y-2" v-if="(props.page && props.page?.published_at) || props.page?.approval?.change_notes">
            ...
            <Drawer>
                ...
                <DrawerContent class="max-w-lg mx-auto">
                    ...
                    <div class="px-4 space-y-4">
                        ...
                        <div class="flex items-center justify-between" v-if="hasPermission('approve_page')">
                            <Label for="approve-directly">Approve Directly</Label>
                            <Switch id="approve-directly" v-model="form.approve_directly" />
                        </div>
                        <div class="flex items-center justify-between" v-if="hasPermission('publish_page')">
