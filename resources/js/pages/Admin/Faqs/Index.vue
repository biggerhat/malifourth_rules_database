...
import FaqView from "@/pages/Rules/FaqView.vue";
...
const columns: ColumnDef<any>[] = [
    ...
    },
    {
        ...
        cell: ({ row }) => {
            ...
            if (faq.internal_notes) {
                return h('div', {}, h(AdminInternalNotes, {
                    title: faq.title,
                    internal_note: faq.internal_notes,
                }));
            }
        ...
        cell: ({ row }) => {
            ...
            return h('div', {}, faq.approval?.approved_at ? h(Check, {class: 'text-green-500 mx-auto'}) : h(Ban, {class: 'text-red-500 mx-auto'}))
        },
    },{
        id: 'actions',
        enableHiding: false,
        ...
        cell: ({ row }) => {
            const faq = row.original;
            const publishable = faq.approval?.approved_at && !faq.published_at;
...
const selectedIds = computed(() =>
    Object.keys(rowSelection.value).map(idx => props.faqs[Number(idx)]?.id).filter(Boolean)
);
...
<template>
    ...
    <div class="container mx-auto mt-6">
        <div class="flex items-center justify-between py-4">
            ...
            <Button @click="router.get(route('admin.faqs.create'))" v-if="hasPermission('add_faq')">
                Create New FAQ
            </Button>
        </div>
        <BulkActionBar
            ...
            model-name="faq"
            :bulk-approve-route="route('admin.faqs.bulk-approve')"
            :bulk-publish-route="route('admin.faqs.bulk-publish')"
            :bulk-delete-route="route('admin.faqs.bulk-delete')"
